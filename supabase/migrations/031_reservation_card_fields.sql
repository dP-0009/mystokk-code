-- ============================================================
-- 031_reservation_card_fields.sql
-- Surfaces two fields the rebuilt Reservation Hub card needs:
--   unit     — the item's unit (e.g. "barrels", "pcs"), for "30 barrels"
--   message  — the reservation's request/remark text, shown on the card
--
-- Both RPCs are between the two reservation parties (requester + responder),
-- who already know each other — `message` is each side's own correspondence,
-- so this is NOT a privacy-chain exposure (no original-owner hiding involved).
-- `unit` is a plain item attribute.
--
-- Adding OUT columns changes the return type, so DROP first. Bodies are the
-- latest live versions (incoming from 022, outgoing from 019) with i.unit +
-- r.message appended.
-- ============================================================

DROP FUNCTION IF EXISTS public.get_reservations_incoming();
CREATE OR REPLACE FUNCTION public.get_reservations_incoming()
RETURNS TABLE (
  reservation_id        uuid,
  share_id              uuid,
  inventory_id          uuid,
  quantity              numeric,
  offered_price         numeric,
  status                text,
  created_at            timestamptz,
  counterparty_company  text,
  item_title            text,
  currency              text,
  list_price            numeric,
  is_middleman          boolean,
  latest_round          int,
  latest_counter_price  numeric,
  latest_counter_qty    numeric,
  passthrough_status    text,
  unit                  text,
  message               text
)
LANGUAGE sql SECURITY DEFINER STABLE SET search_path = public, pg_temp
AS $$
  SELECT
    r.reservation_id, r.share_id, r.inventory_id, r.quantity, r.offered_price, r.status, r.created_at,
    rq.company_name, i.title,
    COALESCE(sh.forward_currency, i.currency), COALESCE(sh.forward_price, i.price),
    (i.vendor_id <> auth.uid()),
    lr.round_number, lr.counter_price, lr.counter_quantity,
    (SELECT child.status FROM reservations child
      WHERE child.parent_reservation_id = r.reservation_id
      ORDER BY child.created_at DESC LIMIT 1) AS passthrough_status,
    i.unit, r.message
  FROM reservations r
  JOIN vendors rq ON rq.id = r.requester_id
  JOIN inventory i ON i.inventory_id = r.inventory_id
  LEFT JOIN shares sh ON sh.share_id = r.share_id
  LEFT JOIN LATERAL (
    SELECT nr.round_number, nr.counter_price, nr.counter_quantity FROM negotiation_rounds nr
    WHERE nr.reservation_id = r.reservation_id ORDER BY nr.seq DESC LIMIT 1
  ) lr ON true
  WHERE r.responder_id = auth.uid()
    AND r.status IN ('pending', 'negotiating', 'passed')
  ORDER BY r.created_at DESC;
$$;
REVOKE ALL ON FUNCTION public.get_reservations_incoming() FROM anon, public;
GRANT EXECUTE ON FUNCTION public.get_reservations_incoming() TO authenticated;

DROP FUNCTION IF EXISTS public.get_reservations_outgoing();
CREATE OR REPLACE FUNCTION public.get_reservations_outgoing()
RETURNS TABLE (
  reservation_id       uuid,
  share_id             uuid,
  inventory_id         uuid,
  quantity             numeric,
  offered_price        numeric,
  status               text,
  created_at           timestamptz,
  counterparty_company text,
  item_title           text,
  currency             text,
  list_price           numeric,
  latest_round         int,
  latest_counter_price numeric,
  latest_counter_qty   numeric,
  unit                 text,
  message              text
)
LANGUAGE sql SECURITY DEFINER STABLE SET search_path = public, pg_temp
AS $$
  SELECT
    r.reservation_id, r.share_id, r.inventory_id, r.quantity, r.offered_price, r.status, r.created_at,
    rs.company_name AS counterparty_company,
    i.title,
    COALESCE(sh.forward_currency, i.currency) AS currency,
    COALESCE(sh.forward_price, i.price)       AS list_price,
    lr.round_number, lr.counter_price, lr.counter_quantity,
    i.unit, r.message
  FROM reservations r
  JOIN vendors rs    ON rs.id = r.responder_id
  JOIN inventory i   ON i.inventory_id = r.inventory_id
  LEFT JOIN shares sh ON sh.share_id = r.share_id
  LEFT JOIN LATERAL (
    SELECT nr.round_number, nr.counter_price, nr.counter_quantity
    FROM negotiation_rounds nr
    WHERE nr.reservation_id = r.reservation_id
    ORDER BY nr.round_number DESC, nr.created_at DESC
    LIMIT 1
  ) lr ON true
  WHERE r.requester_id = auth.uid()
  ORDER BY r.created_at DESC;
$$;
REVOKE ALL ON FUNCTION public.get_reservations_outgoing() FROM anon, public;
GRANT EXECUTE ON FUNCTION public.get_reservations_outgoing() TO authenticated;
