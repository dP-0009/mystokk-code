-- ============================================================
-- 034_received_card_stock_location.sql
-- Surfaces the item's stock_location on the Received Inventory card
-- (the /received list), so each card can show "📍 Dubai, UAE".
--
-- stock_location is an item ATTRIBUTE, not owner identity — the privacy
-- guarantees of safe_received_shares are unchanged:
--   * still OMITS original_owner_id and the raw original price (display_*
--     keeps the forward-price-wins logic);
--   * still joins only the SOURCE vendor's PUBLIC profile.
-- It is already exposed to recipients by get_received_share (the detail RPC,
-- migration 030), so the list view is now consistent with it.
--
-- CREATE OR REPLACE VIEW can only APPEND columns (Postgres forbids inserting
-- or reordering existing view columns), so 033's 24 columns are re-stated in
-- their exact order and stock_location is appended at the end. The RPC selects
-- `*`, so column position is irrelevant to callers; the TS client reads by
-- name. security_invoker (set in 005) is re-asserted so RLS is preserved.
-- ============================================================
...
SELECT
  ...
  -- (033): first photo's storage path.
  ph.storage_path AS first_photo_path,
  -- NEW (034): stock location, appended at the end.
  i.stock_location
FROM shares s
