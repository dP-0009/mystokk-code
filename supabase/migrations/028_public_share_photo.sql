-- 028_public_share_photo.sql
-- B4 privacy rule for the OG card: the image must be the item's FIRST product
-- photo (visual ID is acceptable to expose), never the upstream owner and never
-- price. Add first_photo_path (storage_path of the lowest sort_order photo) so
-- the unfurl endpoint can sign + serve it. Still no original_owner_id, no price
-- string — the existing columns already cover "immediate sender" identity.
-- RETURNS TABLE changes, so drop + recreate + re-grant.
