/**
 * Full list of world countries/territories with their international dial codes.
 * Powers the PhoneField country-code picker and the Country → dial-code auto-fill.
 *
 * `name` matches the labels used in COUNTRIES (industries.ts) so a selected
 * country can be looked up here by name. Codes are E.164 country calling codes.
 */
...
/** Dropdown labels like "United Arab Emirates (+971)", sorted A→Z. */
export const DIAL_OPTIONS: readonly string[] = COUNTRY_DIALS.map((c) => `${c.name} (${c.dial})`);

/** The dial code for a country name (exact match), or undefined. */
export function dialForCountry(name: string | null | undefined): string | undefined {
  if (!name) return undefined;
  return COUNTRY_DIALS.find((c) => c.name === name)?.dial;
}

/** Parse a "Name (+code)" dropdown label back to just the dial code. */
export function dialFromOption(option: string): string {
  const m = option.match(/\(([^)]+)\)\s*$/);
  return m ? m[1] : option;
}

/** The "Name (+code)" option label for a given dial code (first match). */
export function optionForDial(dial: string): string | undefined {
  return DIAL_OPTIONS.find((o) => dialFromOption(o) === dial);
}

/**
 * Split a stored E.164-ish phone string ("+971526630872") into { dial, number }
 * by matching the LONGEST known dial-code prefix. Falls back to an empty dial.
 */
export function splitPhone(full: string | null | undefined): { dial: string; number: string } {
  const value = (full ?? '').trim();
  if (!value) return { dial: '', number: '' };
  if (!value.startsWith('+')) return { dial: '', number: value.replace(/[^\d]/g, '') };

  const digits = value.replace(/[^\d]/g, '');
  // Sort dial codes longest-first so "+1868" wins over "+1".
  const sorted = [...COUNTRY_DIALS].sort((a, b) => b.dial.length - a.dial.length);
  for (const c of sorted) {
    const codeDigits = c.dial.replace('+', '');
    if (digits.startsWith(codeDigits)) {
      return { dial: c.dial, number: digits.slice(codeDigits.length) };
    }
  }
